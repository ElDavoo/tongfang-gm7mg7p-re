...
unchanged.
Every file in the directory is constructed. The `.txt` dumps carry a
`constructed` header saying no EC was read; the `.csv` captures open on their
column line, which is the format the tool reads, and their rows carry the
placeholder `2026-01-01` timestamps. `../test_grade_0751_isolation.py` asserts
that the list in §6 and `0751-isolation-run/` are the same set, so a rename on
