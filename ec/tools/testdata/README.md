...
with no row, a row with no file, a `Feeds` cell naming a tool that is gone, a
nested row naming a listing that is not — that it holds.

`../check_testdata_row_claims.py` reads the **third** column, which the two
...
to rather than per file. Six shapes are counted, printed with the reason and do
not fail the run — "not checked, not absent" — and the rest of the column is
free prose that no rule reaches. Its wiring is prepared at
