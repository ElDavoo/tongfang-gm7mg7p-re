...
The inputs are the four committed text files the default mode reads, and
nothing else:
...
No firmware image, no Ghidra, no network, which is what lets this live in
...
generated symbol table before exporting, so the 56 addresses
...
def occurrence_re(symbols: dict):
    ...
    none of the 56 collides with a decompiled local, a parameter or a
