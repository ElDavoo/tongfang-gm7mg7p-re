// pd @ 9988   set_multiplier_60   [named]
// Ghidra 12.1.3 decompile, generated by ec/tools/build_ec_decompile.py -- do not edit.
// Source: ec/firmware/GMxMGxx_11.800, the ITE8850-PD image at file 0x20000 -- a separate program with its own address space, not a third bank, SHA-256 158d1c6416426939a814146b766a44e2ff0e9286b0abd237e70e51a0c03399c4
// Machine output carrying this repository's symbols. Not the vendor's source.


/* One instruction: B = 0x60. No multiply, store or ret appears here; the C for these two entries
   shows a call to a routine the export names add_full_product_to_dptr, which is not in this
   listing, so what the loaded multiplier is used for is not determined from these bytes.
   type: math
   evidence: ec/decompiled/pd/9988.asm; ec/decompiled/pd/9988.c
   basis: hand-decoded */

void set_multiplier_60(void)

{
                    /* WARNING: Subroutine does not return */
  add_full_product_to_dptr(0x60);
}


