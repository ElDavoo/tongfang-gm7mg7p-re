// bank1 @ 801D   branch_if_acc_zero_to_8021   [named]
...
/* Jumps to 0x8021 when the accumulator is zero, skipping the `dec A` and `movx @DPTR, A` that the
   .c shows following it at 0x801F-0x8020. This listing is that one branch only, inside the body
   running 0x8018 to the `ret` at 0x8189, so the boundary is the call-target byte scan's hypothesis
