...
/* Three instructions: decrement the accumulator, write it back to the XDATA byte DPTR points at,
   then load DPTR with 0x0637, which is the XDATA address in use from here on. Inside the body
   running 0x8018 to the `ret` at 0x8189, so this three-instruction boundary is the call-target byte
