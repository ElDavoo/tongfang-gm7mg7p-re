// bank1 @ 8020   store_acc_to_xdata_dptr_8020   [named]
...
/* The same single instruction as at 0x8018: `movx @DPTR, A`, writing the accumulator to the XDATA
   byte DPTR points at, an address these bytes do not fix. Inside the body running 0x8018 to the
   `ret` at 0x8189, whose .c here decompiles all of it, so the one-instruction boundary is the
   call-target byte scan's hypothesis and the function-level role here is not determined. CORRECTION
   ...
   address is named by 2 of the 180, best framing 2 of 24. Method and the full table:
...
void store_acc_to_xdata_dptr_8020(undefined1 a_from_801f,undefined1 *param_2,char param_3)
