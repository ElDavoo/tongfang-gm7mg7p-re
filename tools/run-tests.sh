...
# test_ec_validate.py export only Ec, test_ec_watch.py and
# test_system_id_probe.py export Ec and EcError, and ec_watch.py does
# `from ecrw import Ec, EcError`. In one shared interpreter, whichever suite
# imports first wins that setdefault, and the narrow one dies on
...
# Per-directory isolation does not help either: all four share one directory.
