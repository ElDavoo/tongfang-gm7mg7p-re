...
file committed. **There are forty-two today, 1288 tests in all** — both figures
*(Superseded by #1009, with `1264 + 24 = 1288` and `41 → 42` the whole of it:
its `ec/tools/test_check_history_checkouts.py` is one new suite at 24 cases, so
this is the fourth merge in the run of notes below to add a suite rather than
cases to one, and the first of them to move both figures by a single suite's
whole. The `forty-one` and `1264` stay written here per
[`../docs/findings.md`](../docs/findings.md) §4a-4d, each true of the tree it
was measured on. The red set is unmoved: the last line reads `42 suite(s) run,
1288 tests; one or more FAILED` with the runner exiting 1, and the red one is
still only `ec/tools/test_check_cluster_citations.py`, 48 tests, on the same
`:220` of the same #822 file with the same two `0x0464`/`0x0465` disagreements
— re-checked here against a pristine `git archive HEAD` extraction, where it
fails identically, so #1009 neither caused it nor fixes it.)*
