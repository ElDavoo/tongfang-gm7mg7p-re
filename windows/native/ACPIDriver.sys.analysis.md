...
`MMRD` (`0x1400015EC`), the handler for `IOCTL 0x9C40A494`, is the same
routine one width up — a 32-bit read of a full physical address rather than an
8-bit read of an EC offset:
...
140001643:  lea    0x28(%rsi),%r9d        ; input length  = 0x28
140001647:  mov    %r9d,0x58(%rsp)
14000164c:  lea    0x4(%rsi),%r14d        ; r14d = 4, the copy-back length
140001650:  mov    (%rbx),%al             ; SystemBuffer[0] -- address, low
140001652:  mov    %al,0x64(%rsp)         ;   ... into Argument[0].Argument[0]
140001656:  mov    0x1(%rbx),%al          ; SystemBuffer[1]
140001659:  mov    %al,0x65(%rsp)
14000165d:  mov    0x2(%rbx),%al          ; SystemBuffer[2]
140001660:  mov    %al,0x66(%rsp)
140001664:  mov    0x3(%rbx),%al          ; SystemBuffer[3]
140001667:  mov    %al,0x67(%rsp)         ;   ... into Argument[0].Argument[3]
140001670:  movl   $0x14,0x28(%rsp)       ; output length = 0x14
14000167d:  movl   $0x43696541,0x50(%rsp) ; ACPI_EVAL_INPUT_BUFFER_COMPLEX_SIGNATURE 'CieA'
140001685:  call   0x140001e88            ; SendDownStreamIrp
140001690:  cmpl   $0x426f6541,0x30(%rsp) ; ACPI_EVAL_OUTPUT_BUFFER_SIGNATURE 'AeoB'
14000169a:  cmpl   $0x0,0x38(%rsp)        ; ...and a non-empty value
1400016a0:  mov    0x40(%rsp),%al         ; Argument[0].Argument[0]
1400016a4:  mov    %r14d,(%rdi)           ; Irp->IoStatus.Information = 4
1400016a7:  mov    %al,(%rbx)             ; back into SystemBuffer[0]
1400016a9:  mov    0x41(%rsp),%al
1400016ad:  mov    %al,0x1(%rbx)
1400016b0:  mov    0x42(%rsp),%al
1400016b4:  mov    %al,0x2(%rbx)
...
`SystemBuffer[0..3]` become one `ACPI_METHOD_ARGUMENT` — four load/store pairs,
`0x140001650`-`0x140001667` — and four bytes of the return value come back
into those same four bytes, `0x1400016A0`-`0x1400016BB`, with
`IoStatus.Information = 4` as in `ECRR`. The listing is checked against
`../decompiled/native/ACPIDriver.asm` (`FUN_1400015ec`) and
[`../decompiled/native/ACPIDriver.c`](../decompiled/native/ACPIDriver.c)
(`FUN_1400015ec`, the same routine in C), both generated by
`../tools/decompile_native.py` and neither hand-edited.

**The address is a full 32-bit physical address, not an EC offset**, which is
the one thing that differs in kind rather than in width: `ECRR` adds
`0xFE410000` in ASL (`evidence/acpi/dsdt.dsl:50497`) where `MMRD` passes
`Arg0` straight to `MMRW`, which opens its `OperationRegion` at exactly the
address it is handed (`:50420`, `:50481`). So a caller of `MMRD` reads at
`0xFE410000 + <EC offset>` and has to add that itself —
`../tools/ecrw.py`'s `read_dword` is where it does, and `readmany` builds
aligned 4-byte blocks over a range for the same reason.

Which of the four returned bytes is the lowest address is the ACPI driver's
arrangement rather than the vendor's: the copy-back is four byte stores out of
the output buffer, in the order that buffer holds them, and ACPI materialises
an integer result little-endian. The same shape carries `ECRR`'s single byte,
where `buf[0]` is the byte at the address — that is the path
`../tools/ecrw.py` has actually exercised — so `buf[0]` is the lowest of the
four here as well. Nothing in this repository has issued an `MMRD`, so that
ordering is a reading of the marshalling and not a measured one. The
...
supporting context that this family is meant to work: the vendor ships a
caller for the byte version. That the 32-bit member works is an inference from
the handler's shape, which is as far as either file goes.
...
- **`MMRD` is established as far as its own code and no further.** That the
  handler marshals four bytes and copies four back is a static fact about
  `0x1400015EC`. What the BIOS returns for a 32-bit read of the EC window, in
  what byte order, and whether a 4-byte access to a window that stalled the
  fans on a sibling board when read a byte at a time (#94) does anything
  different, are all unmeasured here — a readback that matches four `ECRR`
  reads at one address would not settle the last of those either. The
  `--block` path in `../tools/ecrw.py` exists to make that comparison cheap
  for a human with the machine, and it is off by default.
...
- `MMRD`/`MMWD` have IOCTLs and DSDT methods but no `ACPIDriverDll.dll`
  ...
  vestigial.
- **These functions are still `FUN_…`.** The Windows project has no
  `ghidra-functions.csv` (CLAUDE.md: the Windows project does not have one
  yet), so `FUN_1400015ec` is a Ghidra-generated name in a generated `.c`
  rather than an annotation, and every VA in this file has to be quoted
  instead of looked up by name. Creating that CSV is a separate change with a
  wider blast radius than #147's question needs — it regenerates the
  decompile tree and its manifest — and it is where `MMRD`'s handler,
  `ECRR`'s and the other eighteen should get names that say what they are.
