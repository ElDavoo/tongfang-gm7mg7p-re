...
internal class _2p1ndKeyboard_KC : HIDKeyboard
{
	public _2p1ndKeyboard_KC(LM_Manager lm_Manger, string fwVersion)
	...
	{
		if (ite_light > 0 && ite_light <= 25)
		{
			return 1u;
		}
		if (ite_light > 25 && ite_light <= 50)
		{
			return 2u;
		}
		if (ite_light > 50 && ite_light <= 75)
		{
			return 3u;
		}
		if (ite_light > 75 && ite_light <= 100)
		{
			return 4u;
		}
		return 0u;
	...
	public override bool SetEffect(dynamic data)
	{
		...
		SAVE_LIGHTING_EFFECT_DATA sAVE_LIGHTING_EFFECT_DATA = new SAVE_LIGHTING_EFFECT_DATA
		{
			...
			save_light = Convert.ToByte(value),
		...
		if (CurrentPowerMode == RGBKB_PowerMode.DC)
		{
			_DC_LightLevel = _EffectData.save_light;
		}
		else
		{
			_AC_LightLevel = _EffectData.save_light;
		}
		return true;
	}
	...
	internal override void SetBrightnessStrategy(byte brightness, bool saveACDC = true)
	{
		if (saveACDC)
		{
			if (CurrentPowerMode == RGBKB_PowerMode.DC)
			{
				_DC_LightLevel = brightness;
			}
			else
			{
				_AC_LightLevel = brightness;
			}
	...
	public override async void InitalizeWelcomeKeyboardEffect(RGBKB_PowerStatus power, bool IsAppExists, bool unplug = false)
	{
		try
		{
			...
			if (unplug)
			{
				...
				SetBrightnessStrategy((byte)_DC_LightLevel);
				_EffectData.save_light = (byte)_DC_LightLevel;
				SaveEffectData();
			...
			else
			{
				...
				SetBrightnessStrategy((byte)_AC_LightLevel);
				_EffectData.save_light = (byte)_AC_LightLevel;
				SaveEffectData();
	...
	{
		byte brightness = (byte)_DC_LightLevel;
		CurrentPowerMode = RGBKB_PowerMode.DC;
	...
	{
		byte brightness = (byte)_AC_LightLevel;
		CurrentPowerMode = RGBKB_PowerMode.AC;
		PluggedSetBrightness(brightness);
		if (_KeyboardControl.ILM_RGBKB_GetRGBKeyboardType() == RGBKB_Type.FourZone && _EffectData.save_effect == 34)
		{
	...
	internal override void SetACDCLight(bool unplug, uint aclevel, uint dclevel)
	{
		...
		{
			CurrentPowerMode = RGBKB_PowerMode.AC;
			byte b2 = Convert.ToByte(aclevel);
	...
	public override void SetBrinessByScanCode(int scancode)
	{
		...
		case 178:
		{
			...
			uint num = _EffectData.save_light;
			bool flag = true;
			switch (scancode)
			{
			case 177:
				num = ((num >= 25) ? (num - 25) : 0u);
				break;
			case 178:
				if (num + 25 > 100)
				{
			...
			_EffectData.save_light = Convert.ToByte(num);
			if (flag)
			{
	...
	public override void RunEffct(byte SAVED = 0)
	{
		try
		{
			...
			LogCtrl.Write("Run Effect in KC " + _EffectData.save_effect);
			SAVED = 1;
