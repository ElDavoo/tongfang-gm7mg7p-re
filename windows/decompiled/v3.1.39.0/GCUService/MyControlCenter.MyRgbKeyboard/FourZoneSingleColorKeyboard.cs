...
internal class FourZoneSingleColorKeyboard : HIDKeyboard
{
	public FourZoneSingleColorKeyboard(LM_Manager lm_Manger, string fwVersion)
	...
	public override void RunEffct(byte saved)
	{
		...
		ApplyFourZoneSingleColor_EffectData(ref _EffectData);
		base.RunEffct(saved);
	}

	public void ApplyFourZoneSingleColor_EffectData(ref SAVE_LIGHTING_EFFECT_DATA TmpEffectData)
	{
		TmpEffectData.save_effect = Translate_ITE_EffectIndex(RGBKB_Effect.Single);
		List<Color> obj = new List<Color>
		{
		...
		RGBKB_Color save_layout_color = new RGBKB_Color(bCircular: true, 7u);
		int num = 0;
		foreach (Color item in obj)
		{
			save_layout_color.ColorBuffer[num].ID = (uint)num;
		...
		TmpEffectData.save_layout_color = save_layout_color;
		if (TmpEffectData.save_layout_alphbet == null)
		{
			TmpEffectData.save_layout_alphbet = "";
		}
	}
}
