...
public class FanTable_Manager1p5_CML : FanTable_Manager1p5
{
	...
	private CustomizeTable DefaultFanTable_Gaming = new CustomizeTable(16u, "DefaultFanTable_Gaming");

	private CustomizeTable DefaultFanTable_Office = new CustomizeTable(16u, "DefaultFanTable_Office");

	private CustomizeTable DefaultFanTable_Turbo = new CustomizeTable(16u, "DefaultFanTable_Turbo");

	private CustomizeTable M1T1;

	private CustomizeTable M1T2;

	private CustomizeTable M1T3;

	private CustomizeTable M2T1;

	private CustomizeTable M2T2;

	private CustomizeTable M2T3;

	private bool bInit;

	private bool m_IsNvGpu;

	private List<string> FileNames = new List<string> { "M1T1", "M1T2", "M1T3", "M2T1", "M2T2", "M2T3" };

	public FanTable_Manager1p5_CML()
	{
		m_IsNvGpu = UtilityExtensions.IsNvGpu();
		int projectIdFromEC = EcCtrl.GetProjectIdFromEC();
		string name = Enum.GetName(typeof(ProjectID), projectIdFromEC);
		m_path = m_path + "\\" + name;
	}

	public override bool GetFanTableInit()
	{
		return bInit;
	}

	public override void FanTable_Init()
	{
	...
	public override void SetFanControlRespective(dynamic data)
	{
		try
		{
		...
		{
			LogCtrl.TraceMessage("Exception: " + ex.Message, "SetFanControlRespective", "D:\\_work\\ControlCenter3_1\\Service\\MyControlCenter\\MyControlCenter\\MyFan\\FanTable\\FanTable_Manager1p5_CML.cs", 172);
		}
	}

	public override void GetFanTable(string name)
	{
		CustomizeTable customizeTable = default(CustomizeTable);
		customizeTable = TableTransform(name);
		App.m_MQTTService.Publish("Fan/Table", customizeTable, retain: false);
	}

	public override void SetFanTable(string name)
	{
		SetEcFanControlRespective(TableTransform(name).FanControlRespective);
		SetEcFanTable(TableTransform(name));
	}

	public override async void SetFanTableSetting(dynamic data, bool saveEC)
	{
		try
		{
		...
		{
			LogCtrl.TraceMessage("Exception: " + ex.Message, "SetFanTableSetting", "D:\\_work\\ControlCenter3_1\\Service\\MyControlCenter\\MyControlCenter\\MyFan\\FanTable\\FanTable_Manager1p5_CML.cs", 209);
		}
	}

	public override void RestoreDefaultFanTableAll()
	{
		M1T1 = CopyFanTable(DefaultFanTable_Gaming, "M1T1");
		M1T2 = CopyFanTable(DefaultFanTable_Gaming, "M1T2");
		M1T3 = CopyFanTable(DefaultFanTable_Gaming, "M1T3");
	...
	public override void RestoreDefaultFanTable(dynamic data)
	{
		if (data["Name"] != null)
		{
			...
			case "M1T3":
				...
				break;
			case "M2T1":
				M2T1 = GenerateFanDuty(DefaultFanTable_Office, 0.8, text);
				customizeTable = M2T1;
				break;
			case "M2T2":
				M2T2 = GenerateFanDuty(DefaultFanTable_Office, 0.9, text);
				customizeTable = M2T2;
				break;
			case "M2T3":
			...
			if (customizeTable.CPU != null && customizeTable.GPU != null)
			{
				WriteFanTableToJson(customizeTable, activated: false);
				SetEcFanControlRespective(customizeTable.FanControlRespective);
				SetEcFanTable(customizeTable);
				App.m_MQTTService.Publish("Fan/Table", customizeTable, retain: false);
			}
		}
	}

	public override void GetPLDefaultValue(string name, ref string PL1, ref string PL2, ref string PL1_dc, ref string PL2_dc)
	{
		CustomizeTable customizeTable = default(CustomizeTable);
		customizeTable = TableTransform(name);
		PL1 = customizeTable.PL1;
		PL2 = customizeTable.PL2;
		PL1_dc = customizeTable.PL1_dc;
		PL2_dc = customizeTable.PL2_dc;
	}

	public override void GetGpuFeatureDefaultValue(string name, ref string DB, ref string WM)
	{
		CustomizeTable customizeTable = default(CustomizeTable);
		customizeTable = TableTransform(name);
		DB = ((customizeTable.DB == null) ? "NA" : customizeTable.DB);
		WM = ((customizeTable.WM == null) ? "NA" : customizeTable.WM);
	}

	private void RefreshFanTable(ref CustomizeTable fantable, string name)
	{
		fantable.Name = name;
	}
	...
	private void WriteFanTableToJson(CustomizeTable fantable, bool activated)
	{
		try
		{
		...
		{
			LogCtrl.TraceMessage("Exception: " + ex.Message, "WriteFanTableToJson", "D:\\_work\\ControlCenter3_1\\Service\\MyControlCenter\\MyControlCenter\\MyFan\\FanTable\\FanTable_Manager1p5_CML.cs", 348);
		}
	}

	private CustomizeTable LoadFanTableFromJson(string fileName)
	{
		CustomizeTable result = default(CustomizeTable);
		dynamic val = LocalFileLoader.LoadJsonFromPath_NonAsync<object>(m_path, fileName, Assembly.GetExecutingAssembly());
		if (val != null)
		{
			return JsonConvert.DeserializeObject<CustomizeTable>(Json.Stringify(val));
		}
		return result;
	}

	private void GetEcFanTable(ref CustomizeTable fantable)
	{
		byte Data = 0;
		for (int i = 0; i <= 15; i++)
		{
	...
	private void SetEcFanTable(CustomizeTable fantable)
	{
	...
	private void SetEcFanTable_Cpu(CustomizeTable fantable)
	{
		if (fantable.CPU == null)
		{
			return;
	...
	private void SetEcFanTable_Gpu(CustomizeTable fantable)
	{
	...
	private void SetFanTableSetting(CustomizeTable fanTable, string type, int id, string key, int value)
	{
	...
	private void SetFanTableSetting(CustomizeTable fanTable, string type, dynamic data, bool saveEC)
	{
		if (type == "CPU")
		{
	...
	private CustomizeTable GenerateFanDuty(CustomizeTable defaultFantable, double ratio, string name)
	{
		byte b = 0;
		CustomizeTable result = new CustomizeTable(16u, name);
	...
	private CustomizeTable CopyFanTable(CustomizeTable defaultFantable, string name)
	{
		CustomizeTable result = new CustomizeTable(16u, name);
		result.CpuTemp_DefaultMaxLevel = defaultFantable.CpuTemp_DefaultMaxLevel;
		result.GpuTemp_DefaultMaxLevel = defaultFantable.GpuTemp_DefaultMaxLevel;
	...
	public override void ClearFanTableAll()
	{
		byte data = 0;
		for (int i = 0; i <= 15; i++)
		{
	...
	private bool RefreshDefaultFanTable(ref CustomizeTable fantable, byte mode)
	{
		bool flag = false;
		...
		do
		{
			...
			if (IsReadyToRead())
			{
				...
				LogCtrl.TraceMessage("GetFanTable success.", "RefreshDefaultFanTable", "D:\\_work\\ControlCenter3_1\\Service\\MyControlCenter\\MyControlCenter\\MyFan\\FanTable\\FanTable_Manager1p5_CML.cs", 738);
				return true;
			}
			num++;
		}
		while (num != 3);
		LogCtrl.TraceMessage("GetFanTable fail.", "RefreshDefaultFanTable", "D:\\_work\\ControlCenter3_1\\Service\\MyControlCenter\\MyControlCenter\\MyFan\\FanTable\\FanTable_Manager1p5_CML.cs", 746);
		return false;
	}

	private CustomizeTable TableTransform(string name)
	{
		return name switch
		{
			"M1T1" => M1T1, 
	...
	private int GetCpuTemMaxLevel(ref CustomizeTable fantable)
	{
		int num = 10;
		for (int i = 1; i <= 15; i++)
		{
	...
	private int GetGpuTemMaxLevel(ref CustomizeTable fantable)
	{
		int num = 10;
		for (int i = 1; i <= 15; i++)
		{
