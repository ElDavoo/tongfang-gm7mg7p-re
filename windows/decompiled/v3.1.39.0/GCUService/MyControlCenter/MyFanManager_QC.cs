...
public class MyFanManager_QC : MyFanManager
{
	...
	private uint m_BenchmarkMode;

	private uint m_BenchmarkModeSupport;

	private uint m_EnableFanDutyCycleReduction_Default;

	private uint m_DisablePassiveCoolingMode;

	private uint m_DisablePassiveCoolingMode_Default;

	private uint m_BatteryChargingLimit = 100u;

	private uint m_BatteryChargingLimit_Default = 100u;

	private List<uint> m_BatteryChargingLimitItems = new List<uint> { 60u, 70u, 80u, 90u, 100u };
	...
	public override async void Receive(byte[] data)
	{
		...
		case "BENCHMARK_OFF":
			...
			UpdateStatusToClient(1);
			break;
		case "DISABLE_PASSIVECOOLING_MODE_ON":
		case "DISABLE_PASSIVECOOLING_MODE_OFF":
			UserSet_DisablePassiveCooloing(text);
			UpdateStatusToClient(1);
			break;
		case "BATTERY_CHARGINGLIMIT_SETTING":
			UserSet_BatteryChargingLimit_Setting(text, val);
			UpdateStatusToClient(1);
			break;
		}
	}
	...
	public override void UpdateStatusToClient(int autosave, int cpuusage = 0, int cputemp = 0, bool updateOperationMode = true)
	{
		if (autosave != 0)
		{
			var data = new
			{
				...
				DisablePassiveCoolingMode = m_DisablePassiveCoolingMode.ToString(),
				BatteryChargingLimit = m_BatteryChargingLimit.ToString()
			};
	...
	public override void Resume()
	{
		...
		if (SMBIOSINFO.m_bSupportBatteryLifeExtension)
		{
			m_BatteryChargingLimit = ReadBatteryChargingLimit();
		}
		if (SMBIOSINFO.m_bSupportFanDutyCycle)
		{
			m_DisablePassiveCoolingMode = GetFanNeverResolveToZero();
		}
		Init();
	}
	...
	private void UserSet_SysPowerMode_Setting(string action, dynamic data)
	{
		...
		if (data["FanMode"] != null)
		{
			...
			if (num >= 1 && num <= 3)
			{
				...
				{
				case "SYSPOWER_PERFORMANCE_SETTING":
					cDefaultProfile1.F = num;
					SetUserProfileRegistry("Index1", "F", num);
					break;
				case "SYSPOWER_BALANCED_SETTING":
					cDefaultProfile2.F = num;
					SetUserProfileRegistry("Index2", "F", num);
					break;
				case "SYSPOWER_BATTERYSAVER_SETTING":
					cDefaultProfile3.F = num;
					SetUserProfileRegistry("Index3", "F", num);
					break;
				}
			}
		}
	...
	private void UserSet_DisablePassiveCooloing(string action)
	{
		if (!SMBIOSINFO.m_bSupportFanDutyCycle)
		{
			return;
		}
		if (!(action == "DISABLE_PASSIVECOOLING_MODE_ON"))
		{
			if (action == "DISABLE_PASSIVECOOLING_MODE_OFF")
			{
				m_DisablePassiveCoolingMode = 0u;
			}
		}
		else
		{
			m_DisablePassiveCoolingMode = 1u;
		}
		SetFanNeverResolveToZero(m_DisablePassiveCoolingMode);
	}

	private void UserSet_BatteryChargingLimit_Setting(string action, dynamic data)
	{
		if (SMBIOSINFO.m_bSupportBatteryLifeExtension && ((data["Limit"] != null) ? true : false))
		{
			uint value = Convert.ToUInt32(data["Limit"]);
			m_BatteryChargingLimit = value.CheckIntegrityValue(100u, m_BatteryChargingLimitItems);
			SetBatteryChargingLimit(m_BatteryChargingLimit);
		}
	}
	...
	private void LoadRegistry()
	{
		...
		try
		{
			...
			num2 = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_RegistryPath, "BenchmarkMode", 0);
			num3 = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_RegistryPath, "FirstTime", 0);
			m_SysPowerModeIndex = (uint)num;
			m_BenchmarkMode = (uint)num2;
			m_FirstTime = (uint)num3;
	...
	private void LoadDefault()
	{
		...
		SetRegistry("BenchmarkMode", m_BenchmarkMode);
		m_FirstTime = 1u;
		SetRegistry("FirstTime", m_FirstTime);
		...
		m_DisablePassiveCoolingMode = m_DisablePassiveCoolingMode_Default;
		m_BatteryChargingLimit = m_BatteryChargingLimit_Default;
	}
	...
	private void LoadDefaultFromSetupINI()
	{
		try
		{
			int benchmarkModeSupport = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, "\\OEM\\GamingCenter2\\Features", "BenchmarkModeSupport", 0);
			int enableFanDutyCycleReduction_Default = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, "\\OEM\\GamingCenter2\\Features", "EnableFanDutyCycleReduction", 0);
			int disablePassiveCoolingMode_Default = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, "\\OEM\\GamingCenter2\\Features", "DisablePassiveCoolingMode", 0);
			int batteryChargingLimit_Default = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, "\\OEM\\GamingCenter2\\Features", "BatteryChargingLimit", 100u);
			m_BatteryChargingLimit_Default = m_BatteryChargingLimit_Default.CheckIntegrityValue(100u, m_BatteryChargingLimitItems);
			m_BenchmarkModeSupport = (uint)benchmarkModeSupport;
			m_EnableFanDutyCycleReduction_Default = (uint)enableFanDutyCycleReduction_Default;
			m_DisablePassiveCoolingMode_Default = (uint)disablePassiveCoolingMode_Default;
			m_BatteryChargingLimit_Default = (uint)batteryChargingLimit_Default;
		}
		catch (Exception ex)
		{
			LogCtrl.Write(className + "[LoadDefaultFromSetupINI] Features Exception" + ex.Message);
		}
		try
		{
			m_nDefaultPowerMode = (uint)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, "\\OEM\\GamingCenter2\\DefaultPower", "PowerMode", 2u);
		...
		{
			LogCtrl.Write(className + "[LoadDefaultFromSetupINI] DefaultPower Exception" + ex2.Message);
	...
	private void Init()
	{
		...
		if (SMBIOSINFO.m_bSupportFanDutyCycle)
		{
			SetFanDutyCycleReduction(m_EnableFanDutyCycleReduction_Default);
		}
	}
	...
	private void SetFanDutyCycleReduction(uint mode)
	{
		byte Data = 0;
		byte b = 223;
		byte b2 = 0;
		byte b3 = 0;
		b2 = (byte)((mode == 1) ? 32 : 0);
		EcCtrl.Read(GetType().Name, 1955, ref Data);
		b3 = (byte)((Data & b) + b2);
		EcCtrl.Write(GetType().Name, 1955, b3);
	}

	private void SetFanNeverResolveToZero(uint mode)
	{
		byte Data = 0;
		byte b = 191;
		byte b2 = 0;
	...
	private uint GetFanNeverResolveToZero()
	{
		byte Data = 0;
		EcCtrl.Read(GetType().Name, 1955, ref Data);
		if ((byte)(Convert.ToUInt64(Data) & 0x40) == 64)
		{
			return 1u;
		}
		return 0u;
	...
	private void FollowBiosIndex()
	{
		if (SMBIOSINFO.m_bSupportQcRefreshRule)
		{
			m_SysPowerModeIndex = GetModeProfileIndexHigh();
			SetRegistry("SysPowerModeIndex", m_SysPowerModeIndex);
			m_CurrentIndex = m_SysPowerModeIndex;
			g_FanMode = GetModeProfileIndexLow();
			switch (m_CurrentIndex)
			{
			case 1u:
				m_CpuMode = cDefaultProfile1.P;
				m_DGpuMode = cDefaultProfile1.G;
				cDefaultProfile1.F = g_FanMode;
				SetUserProfileRegistry("Index1", "F", g_FanMode);
				break;
			case 2u:
				m_CpuMode = cDefaultProfile2.P;
				m_DGpuMode = cDefaultProfile2.G;
				cDefaultProfile2.F = g_FanMode;
				SetUserProfileRegistry("Index2", "F", g_FanMode);
				break;
			case 3u:
				m_CpuMode = cDefaultProfile3.P;
				m_DGpuMode = cDefaultProfile3.G;
				cDefaultProfile3.F = g_FanMode;
				SetUserProfileRegistry("Index3", "F", g_FanMode);
				break;
			}
			return;
		}
	...
	private void SetModeProfileIndexHigh(uint mode)
	{
		byte Data = 0;
		byte b = 15;
		uint num = 32u;
		...
		EcCtrl.Read(GetType().Name, 1963, ref Data);
		b2 = (byte)((Data & b) + num);
		EcCtrl.Write(GetType().Name, 1963, b2);
	}

	private void SetModeProfileIndexLow(uint mode)
	{
		byte Data = 0;
		byte b = 240;
		uint num = 2u;
	...
	private uint GetModeProfileIndexHigh()
	{
		byte Data = 0;
		byte b = 240;
		byte b2 = 32;
	...
	private uint GetModeProfileIndexLow()
	{
		byte Data = 0;
		byte b = 15;
		byte b2 = 2;
	...
	private void SetBatteryChargingLimit(uint limit)
	{
		byte Data = 0;
		byte b = 128;
		byte b2 = 0;
	...
	private uint ReadBatteryChargingLimit()
	{
		byte Data = 0;
		byte b = 127;
		byte b2 = 0;
