...
internal class MyRgbLightbarManager_Intel : MyRgbLightbarManager
{
	...
	private uint m_red_level = 100u;

	private uint m_green_level = 100u;

	private uint m_blue_level = 100u;
	...
	private void SetRedLevel(uint level, bool bWriteToECRam)
	{
		if (bWriteToECRam)
		{
			...
			EcCtrl.Write(GetType().Name, 1865, (byte)(level * 2));
		}
	}
	...
	private void SetGreenLevel(uint level, bool bWriteToECRam)
	{
		if (bWriteToECRam)
		{
			...
			EcCtrl.Write(GetType().Name, 1866, (byte)(level * 2));
		}
	}
	...
	private void SetBlueLevel(uint level, bool bWriteToECRam)
	{
		if (bWriteToECRam)
		{
			...
			EcCtrl.Write(GetType().Name, 1867, (byte)(level * 2));
		}
	}
	...
	private void LoadRegistry()
	{
		try
		{
			int num = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_path, "RedLevel", 100u);
			int num2 = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_path, "GreenLevel", 100u);
			int num3 = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_path, "BlueLevel", 100u);
			...
			if ((long)num < 0L || (long)num > 100L)
			{
				RegistryCtrl.RegistrySoftwareKeyWrite(RegistryHive.LocalMachine, m_path, "RedLevel", 100u, RegistryValueKind.DWord);
				num = 100;
			}
			if ((long)num2 < 0L || (long)num2 > 100L)
			{
				RegistryCtrl.RegistrySoftwareKeyWrite(RegistryHive.LocalMachine, m_path, "GreenLevel", 100u, RegistryValueKind.DWord);
				num2 = 100;
			}
			if ((long)num3 < 0L || (long)num3 > 100L)
			{
				RegistryCtrl.RegistrySoftwareKeyWrite(RegistryHive.LocalMachine, m_path, "BlueLevel", 100u, RegistryValueKind.DWord);
				num3 = 100;
			}
			if ((long)num4 < 0L || (long)num4 > 100L)
			{
				RegistryCtrl.RegistrySoftwareKeyWrite(RegistryHive.LocalMachine, m_path, "RedLevel_dc", 0u, RegistryValueKind.DWord);
				num4 = 0;
			}
			if ((long)num5 < 0L || (long)num5 > 100L)
			{
				RegistryCtrl.RegistrySoftwareKeyWrite(RegistryHive.LocalMachine, m_path, "GreenLevel_dc", 0u, RegistryValueKind.DWord);
				num5 = 0;
			}
			if ((long)num6 < 0L || (long)num6 > 100L)
			{
	...
	private void LoadRegistryForLineStatus()
	{
		try
		{
			int red_level = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_path, "RedLevel", 100u);
			int green_level = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_path, "GreenLevel", 100u);
			int blue_level = (int)RegistryCtrl.RegistrySoftwareKeyRead(RegistryHive.LocalMachine, m_path, "BlueLevel", 100u);
