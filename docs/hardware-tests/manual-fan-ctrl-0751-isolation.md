...
- **§6's ten files, all but three of them.** A probe run with `--csv` produces
  ...
  into **one** appended file rather than §6's three — the probe's watch set
  already spans all three ranges (`0x0783`-`0x0785` and `0x07C6` from the
  `0x0700` one, the fan table, the temperature range), so §6's split is
  `ec_watch.py`'s one-console-per-file shape rather than a coverage
  difference — and the grader reads that one file with no conversion. Two
  things follow for the record. The windowed §4.1-§4.3 read therefore
  applies to a probe capture, and §4.6 does not. And with one capture there
  is no other console for a mark to be missing from and no second spelling to
  disagree with it, so the grader's cross-console checks do not run — its
