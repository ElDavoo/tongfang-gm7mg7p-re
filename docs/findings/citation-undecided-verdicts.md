...
| 17 | `bank1,B33B ← bank1,AD68` | "the .c's switch reads those eight targets as 0xB56C, … 0xB33B, …" | **two hops.** `bank1/AD68.asm:12` is `AD70 90 ad 50 mov DPTR,#0xad50` and `:16` is `AD76 73 - - jmp @A+DPTR`; the entry at 0xAD50 + 5×3 is `ljmp 0xAD85` (`bank1/AD50.asm:7` shows the table's own shape), and `bank1/AD85.asm:6` is `AD85 02 b3 3b ljmp 0xb33b` | `code` | the table at 0xAD50 holds ljmps to the forwarder stubs at 0xAD77-0xAD88, and those stubs are the bytes the `.c` switch read. 0xB33B is case 5, reached through `forwarder_to_b33b`. The comment is right, and the seven `forwarder_to_*` listings the eight cases reach are the same fact |
...
- **The 36 `code` verdicts are not 36 new edges.** A `code` verdict says the
  mention names a code address. For the 16 rows where the citing listing
  carries a transfer, that is a call the graph could have counted and did not,
  because the window never reached the verb. For the 12 rows naming another
