...
it: the 382/148/206/28 partition, the 109 ranked callees and 137 citing
...
rather than looking for the number in the report: the 295 comments and 118
kept callees, the 137 of the 148 kept pairs whose callee the table carries a
row for, the collision table, the 158 cross-program pairs left to the frame
test, the 13 weak-marker credits, and the 5,697/1,690 unresolved-mention
census. The `FILLER_BUDGET` variants are
the exception that is re-run rather than read: they come from re-running the
gate with the constant at 2 and at 3.

**The partition above is the one issue #525's citing-listing gate produces, not
the one this file was written against** — 148/206/28 rather than 152/185/45, and
21 of the 206 carry the new `fill-at-citer` reason while 16 of the 148 are kept
on a transfer in the citing listing rather than on a frame. Every figure in this
file that describes the *frame* gate alone is unchanged by that: the frame
census, the reason table below it and the 45 cross-program rejections all
re-derive the same. **The collision set is the one exception and it moved for
this reason**: three callees joined it, `common,0F75`, `common,158E` and
`common,1594`, each losing the seven fill comments the veto refuses, so the
"lost at least three" bar is 20 callees here rather than the 17 this file was
written against. The correction to the follow-up this file left open is in
`citing-listing-evidence.md`; the claims that named the defect as open are
struck and pointed at there, in place.
...
| verdict | pairs | |
|---|---:|---|
| kept — a code frame governs the mention | 148 | enters `cited_by` |
| rejected — a data frame, another program's row, or a fill citing listing | 206 | reported, not dropped |
| undecided — no frame inside the window | 28 | reported, not defaulted |

Those 148 kept pairs are 118 distinct callees, 109 of which the table can rank.
...
| `fill-at-citer` | 21 | the citing listing is an `0xFF` run; issue #525, below |
...
A pair can carry two reasons, which is why the column does not sum to 206. The
frame-gate figures — every reason above the new one, and all three of the
verdict rows for the frame half — are the ones this file measured; the `kept`,
`rejected` and `undecided` totals include issue #525's citing-listing rule,
which is described in `citing-listing-evidence.md`.
...
`citations()` callee key, and **20 callees clear that bar on this tree — the
19 rows below are all of them**, one row carrying the 0x08CD/0x08CF pair. The
three that joined the set since this file was written are `common,0F75`,
`common,158E` and `common,1594`, and they joined it for the reason issue #525
is about rather than for a reading change: each lost the same seven `bank1`
fill comments, which the citing-listing veto now refuses. Re-derivable with:

    python3 -c "import sys,collections; sys.path.insert(0,'ec/tools'); import call_graph as cg; i=cg.load_index(); e,u,o,t,l=cg.scan(i); k,r,d=cg.citations(i,l)[:3]; print(collections.Counter(c.callee for c in r+d).most_common(20))"
...
| `common,158E` | 8 | **1** | 7 | 0 | **all 7 the `bank1` `0xFF`-fill comments** issue #525 is about, refused `fill-at-citer`; the 1 kept is `common,0070`, whose listing carries the `lcall` |
| `common,0F75` | 8 | **1** | 7 | 0 | the same seven fill comments and the same real call behind them |
| `common,1594` | 8 | **1** | 7 | 0 | as the two rows above; the three `common,0070` mentions are one enumeration |
...
| `pd,06EA` | 8 | **2** | 4 | 2 | a 16-bit counter's halves, `0x06EA/0x06EB`; the 2 kept are `pd,88BE`'s "calling 0x05A6 and 0x06EA" and `pd,D83D`'s argument list, whose listing carries the `lcall` at `:38` |
...
them — or, since issue #525, the citing listing's own transfer can. This section
describes the 45 the frame gate alone returned; **17 of them have since left
this population and all 17 are accounted for** — 16 credited from the citing
listing, all in the table below, and the seventeenth (`bank1,9B3C ←
bank1,9B03`) credited by `sjmp` entering the code lexicon in issue #526, which
is the one reclassification in that companion write-up. Three shapes, all
real:
...
- **A real citation across an aside.** `common,0070` reads "calls 0x110A -- the
  ...
  to settle it. **Settled that way since #525** — the same listing corroborates
  all three, and they are now the sole `citing` entry on those rows.
...
- ~~**The top of the table is now right about the addresses and still wrong about
  ...
  fix.~~ **Fixed 2026-09-25, issue #525**:
  [`citing-listing-evidence.md`](citing-listing-evidence.md). The citing
  listing is `0xFF` fill, so `../ec/tools/citation_callers.py` refuses all 21
  pairs as `fill-at-citer` and the three rows read `cited_by=1` against their
  own `inbound=1`, `citing` reduced to `common:0070` — the one real call, whose
  own listing carries the `lcall`. The same listing evidence credits 16 pairs
  this window reported as undecided. The top of the ranking is no longer the
  fill artifact; `ec/annotations/call-graph.md` §"Two known distortions"
  carries the correction in place.
- **A frame test is a bag of words.** It is lexical, bounded, and it will be
  wrong on prose it has not seen. Every one of the 132 pairs the frame gate
  alone kept was read, because a wrong citation enters a work list and a
  settled-looking one does not announce itself — and the 16 the citing-listing
  rule has since added were read one by one in
  [`citing-listing-evidence.md`](citing-listing-evidence.md), which is where
  its one wrong credit is recorded. **14 of the 132 rest on a weak marker**
  rather than on `calls`/`lcall`/`ljmp` — `callee` (2), `tail-jumping` (3),
  `entry` (2), `jumping` (2), `routine` (2), `caller`, `reaches`, `routines`
  (1 each) — and
...
1. ~~**The fill-comment artifact** (above): 21 of the top three's citations are
   ...
   from "names a callee in a body it rejects".~~ **Closed 2026-09-25, issue
   #525** — and note what closed it: the discriminator is not lexical at all,
   which is what this item kept suspecting. The citing listing's bytes are
   (`citation_callers.is_fill`), and the same listing read the other way
   corroborated 16 `undecided` pairs. Write-up in
   [`citing-listing-evidence.md`](citing-listing-evidence.md).
2. ~~**The 45 undecided pairs** want a human reading each sentence, the way
   ...
   **What is left is the 28 above**, and it is 28 rather than the 29 issue
   #525 left because `sjmp` took one of them. Seventeen of the 45 are settled
   in the tool's own terms — 16 by a transfer in the citing listing, one by the
   `sjmp` lexicon word — and every one of the 28 the rest carry a recorded
   reading. A wider code lexicon would still settle more, and would need its
   own evidence for each word it adds.
...
4. **`ec/tools/test_citation_frames.py` has no gate arm.** Its 28 unit tests are
